...
class SoundCorkMediaPlayer(CoordinatorEntity, MediaPlayerEntity):
    """Representation of a SoundCork-managed Bose speaker."""
    ...
    def _data(self) -> dict[str, Any]:
        """Return data for this speaker from the coordinator."""
    ...
    def source(self) -> str | None:
        """Return current source (preset name if playing a preset, else raw source)."""
    ...
    def source_list(self) -> list[str]:
        """Return list of preset names as source options."""
    ...
    async def async_select_source(self, source: str) -> None:
        """Play the preset matching the given source name."""
        for preset in self._presets:
    ...
    async def async_play_media(
        ...
        """Play media. media_id can be a preset number (1-6) or TuneIn station ID."""
        if media_id.isdigit():
    ...
    async def async_store_preset_tunein(
        ...
        """Store a TuneIn station as a preset."""
        xml = (
    ...
    async def async_store_preset_radio(
        ...
        """Store a direct radio stream URL as a preset."""
        xml = (
            f'<preset id="{preset_num}">'
    ...
    async def _select_content_item(self, preset: dict[str, Any]) -> None:
        """Send a select command for a preset content item."""
        xml = (
