    ...
    grid.querySelectorAll('.preset-card').forEach(card => {
      ...
      card.addEventListener('click', () => {
        if (preset) {
          ...
          (async () => {
            try {
              const kn = `PRESET_${preset.id}`;
              await api.speakerPost(ip, 'key', `<key state="press" sender="Gabbo">${kn}</key>`);
              await api.speakerPost(ip, 'key', `<key state="release" sender="Gabbo">${kn}</key>`);
...
function renderEditTuneInPreset(main, ip, presetId) {
  ...
  async function doSearch() {
    ...
    try {
      ...
      const outlines = Array.from(xml.querySelectorAll('outline')).filter(o =>
        o.getAttribute('type') === 'audio' ||
        (o.getAttribute('type') === 'link' && o.getAttribute('item') === 'show')
      );
      if (outlines.length === 0) {
        resultsEl.innerHTML = '<div class="empty-state"><p>No stations or podcasts found</p></div>';
        return;
      }
      resultsEl.innerHTML = '';
      outlines.forEach(o => {
        ...
        const isPodcast = o.getAttribute('item') === 'show';
        ...
        item.innerHTML = `
          ${image
            ...
            : `<div class="list-item-thumb-placeholder">${isPodcast ? '&#x1F3A7;' : '&#x1F4FB;'}</div>`}
          <div class="list-item-body">
            <div class="list-item-title">${escapeHtml(name)}${isPodcast ? '<span style="background:rgba(150,50,255,.3);color:#b388ff;font-size:10px;padding:1px 5px;border-radius:4px;margin-left:4px">podcast</span>' : ''}</div>
